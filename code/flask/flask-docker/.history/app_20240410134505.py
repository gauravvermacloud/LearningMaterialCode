...
logging.basicConfig(
    ...
    format="%(asctime)s ~ %(levelname)s ~  %(threadName)-12.12s ~ %(process)d ~ %(funcName)s ~ %(lineno)d ~ %(message)s",
    encoding="utf-8",
    level=logging.DEBUG,
)
...
def before_request():
    ...
    Response.headers.set("X-Universal-Request-Id", current_thrd.my_prop)
    ...
    if Authorization_Header != None:
        ...
        token_decode = decode(bearer_token, "Hello")
        token_dict = dict(token_decode)
        user = User(
            token_dict.get("user_name"), token_dict.get("roles"), token_dict.get("data")
        )
...
if __name__ == "__main__":
    if config_reader.server == "Flask":
        app.run(
            debug=config_reader.debug, host=config_reader.host, port=config_reader.port
        )
    if config_reader.server == "Waitress":
        serve(app, host=config_reader.host, port=config_reader.port)
    ...
    if config_reader.server == "Gevent":
        ...
        if config_reader.is_ssl_enabled:
            server = pywsgi.WSGIServer(
                (config_reader.host, config_reader.port),
                app,
                keyfile="host.key",
                certfile="host.cert",
            )
        else:
            server = pywsgi.WSGIServer((config_reader.host, config_reader.port), app)
